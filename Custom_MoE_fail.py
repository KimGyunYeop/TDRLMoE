...
class SwitchTransformersConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        num_rl_layers=1,
        num_rl_samples=4,
        rl_router_select_stretegy="multinomial",
        **kwargs,
    ):
        self.num_rl_layers = num_rl_layers
        self.num_rl_samples = num_rl_samples
        self.rl_router_select_stretegy = rl_router_select_stretegy
...
class SwitchTransformersTop1Router(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple:
        ...
        print(f"router_probs: {router_probs}")
        print(f"router_logits: {router_logits}")
        print(f"router_probs shape: {router_probs.shape}")
        ...
        print(f"token_priority: {token_priority}")
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, expert_class: nn.Module = SwitchTransformersDenseActDense):
        ...
        self.num_rl_samples = config.num_rl_samples # added
        self.rl_router_select_stretegy = config.rl_router_select_stretegy # added

    def forward(self, hidden_states, do_rl_block=False): # added
        r"""
        ...
        for idx in idx_mask:
            active_mask = router_mask_bool[:, :, idx]  # (b, s)
            next_states[active_mask] = getattr(self.experts, "expert_{}".format(idx))(
                hidden_states[active_mask]
            )

        top1_hidden_states = router_probs * next_states
        top1_expert_idx = torch.argmax(router_mask, dim=-1)  # (b, s)

        # Round 1에서의 "선택된 확률"은 router_probs.squeeze(-1)로 가능
        # shape (b, s)
        round1_prob_selected = router_probs.squeeze(-1)
        
        # added
        # ==========================
        # Round 2..N: capacity 무시, 이전 Expert 제외 
        # ==========================
        
        extra_outputs = None
        round_experts_list = None
        
        if do_rl_block and (self.num_rl_samples > 1): 
            extra_outputs = []
            round_experts_list = []
            # full distribution (b, s, E)
            full_probs = torch.softmax(router_logits, dim=-1)

            # used_mask: (b, s, E) => Round 1에서 이미 선택된 Expert 기록
            used_mask = router_mask_bool.clone()

            # 계속 반복
            prev_hidden = hidden_states  # 라운드별로 어떤 입력을 forward할지 결정. (demo)
            for round_i in range(2, self.num_rl_samples + 1):
                # 1) 이미 사용된 Expert 확률=0
                round_probs = full_probs.clone()
                round_probs[used_mask] = 0.0

                # 2) argmax
                if self.rl_router_select_stretegy == "multinomial":
                    # round_probs: (b, s, E), used_mask: (b, s, E)
                    # 여기서는 "already used" expert의 prob=0 해서 multinomial하는 방식
                    # (기존 구현 예시)
                    flat_probs = round_probs.view(batch_size * seq_len, num_experts)
                    sampled = torch.multinomial(flat_probs, 1)  # (b*s, 1)
                    new_expert_idx = sampled.view(batch_size, seq_len)

                elif self.rl_router_select_stretegy == "argmax":
                    new_expert_idx = torch.argmax(round_probs, dim=-1)  # (b, s)

                elif self.rl_router_select_stretegy == "random":
                    # ==============================================
                    # "Random" selection while avoiding used experts
                    # ==============================================
                    # 여기서는 round_probs는 사용하지 않고, 완전히 uniform random
                    # 단, used_mask == True인 expert는 제외
                    random_probs = torch.ones_like(round_probs, device=round_probs.device)  # (b, s, E)
                    random_probs[used_mask] = 0.0
                    
                    new_expert_idx = torch.multinomial(random_probs.view(batch_size * seq_len, num_experts), 1)
                    new_expert_idx = new_expert_idx.view(batch_size, seq_len)   
                            
                # 3) forward
                new_mask = torch.nn.functional.one_hot(new_expert_idx, num_classes=num_experts).bool()  # (b, s, E)
                new_states = prev_hidden.clone()

                new_idx_mask = new_mask.view(batch_size*seq_len, num_experts).sum(dim=0)
                new_idx_mask = torch.nonzero(new_idx_mask, as_tuple=True)[0].tolist()
                for ex_idx in new_idx_mask:
                    active_mask = new_mask[:, :, ex_idx]
                    new_states[active_mask] = self.experts[f"expert_{ex_idx}"](prev_hidden[active_mask])
                    
                # 4) 해당 Expert의 확률
                # shape (b, s)
                gather_probs = torch.gather(
                    round_probs, 2, new_expert_idx.unsqueeze(-1)
                ).squeeze(-1)
                new_hidden = gather_probs.unsqueeze(-1) * new_states

                extra_outputs.append(new_hidden)
                round_experts_list.append(new_expert_idx)

                # 5) used_mask 업데이트 (중복 Expert 방지)
                used_mask = used_mask | new_mask

        # extra_outputs: [ (b,s,d), (b,s,d), ... ] for each round 2..N
        return top1_hidden_states, (router_logits, top1_expert_idx), extra_outputs, round_experts_list
...
class SwitchTransformersLayerFF(nn.Module):
    ...
    def forward(self, hidden_states, output_router_logits, do_rl_block=False): #added
        forwarded_states = self.layer_norm(hidden_states)
        if self.is_sparse:
            forwarded_states = self.mlp(forwarded_states, do_rl_block) # added
        else:
            forwarded_states = self.mlp(forwarded_states)

        if isinstance(forwarded_states, tuple):
            forwarded_states, router_tuple, extra_outputs, round_experts_list = forwarded_states
        else:
            router_tuple = None
            extra_outputs = None
            round_experts_list = None
        ...
        if extra_outputs is not None:
            for i in range(len(extra_outputs)):
                extra_outputs[i] += hidden_states + self.dropout(extra_outputs[i])

        if output_router_logits and router_tuple is not None:
            output = (output, router_tuple, extra_outputs, round_experts_list)
...
class SwitchTransformersBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, output_router_logits, do_rl_block) # added

        if isinstance(hidden_states, tuple):
            hidden_states, router_tuple, extra_outputs, round_experts_list = hidden_states
        else:
        ...
        if use_cache:
            outputs = outputs + (past_key_value,) + attention_outputs + ((router_tuple, extra_outputs, round_experts_list))
        else:
            outputs = outputs + attention_outputs + ((router_tuple, extra_outputs, round_experts_list))
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        for i in range(config.num_layers):
            ...
            self.sparse_list.append(is_sparse)
        ...
        #added
        self.num_rl_layers = config.num_rl_layers
        self.num_rl_samples = config.num_rl_samples
    ...
    def forward(
        ...
        #added
        if do_rl and self.training:
            # sparse_list에서 True인 레이어 인덱스만 모음
            sparse_indices = [idx for idx, sp in enumerate(self.sparse_list) if sp]
            if len(sparse_indices) > 0:
                # min(...) => 실제 sparse layer 수보다 rl_layer가 많으면 전체 sparse 레이어 사용
                chosen_count = min(self.num_rl_layers, len(sparse_indices))
                # 무작위로 chosen_count개 뽑음
                selected_layers = random.sample(sparse_indices, chosen_count)
            else:
                selected_layers = []
        else:
            selected_layers = []
        ...
        for i, layer_module in enumerate(self.block):
            ...
            # 이 레이어가 RL 대상 레이어인지 여부
            do_rl_block = (i in selected_layers)
            ...
            if do_rl_block:
                branch_hiddens = layer_outputs[-1][1]
                branch_idx = layer_outputs[-1][2]
                branced_layer_idx = i

            router_probs = layer_outputs[-1][0]
        ...
        if do_rl_block:
            # branch_forward 함수를 통해 선택된 레이어의 hidden_states를 가져옴
            
            for j, branch_hidden_states in enumerate(branch_hiddens):
                branch_hiddens[j] = self.branch_forward(
                    branch_hidden_states, 
                    hidden_states, 
                    branced_layer_idx,
                    ...
